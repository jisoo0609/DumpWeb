...
function addTableRow(data) {
  ...
  searchResultData.slice(0, 5).forEach((data, index) => {
    const row = document.createElement("tr");

    let order = [
      data.fromsite, data.tosite, data.item, data.qty,
      (data.carNo),
      data.qtyup
    ];
    ...
    if (someCondition) {
      // 조건에 따라 rowHTML을 수정할 수 있음
      rowHTML += `<td>Some additional content</td>`;
    }
    ...
    const tableBody = document.querySelector("table tbody");
     ...
     tableBody.addEventListener("click", (event) => {
         ...
         if (parentRow) {
             ...
             if (driveID) {
                 const url = `/dailyReport/orderform?driveID=${driveID}`;
